...
author: "Abood, Mohmand, Wellington"
...
m[, value := ifelse(grepl("^n_", metric),
                    formatC(as.integer(round(as.numeric(value))), format = "d", big.mark = ","),
                    sprintf("%.4f", as.numeric(value)))]
kable(m, caption = "Logistic regression diagnostics.", col.names = c("Metric", "Value"))
```
...
```{r fig4, fig.cap="CART decision tree (loss-weighted to address class imbalance). Each terminal node is labeled with the predicted class and (n\\_NonFatal, n\\_Fatal) counts."}
